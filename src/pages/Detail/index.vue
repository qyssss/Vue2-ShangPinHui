...
export default {
  ...
  computed: {
    ...
    // 给子组件Zoom的数据
    skuImageList() {
      // 如果服务器数据还没来,至少给个空数组,否则子组件得到undefined会有问题
  ...
  methods: {
    ...
    // 加入购物车的回调
    async addShopCart(){
      // 1.发请求给服务器,这里实际上是调用仓库中的函数,并且返回的是Promise,
      // 这里await等待仓库中返回的Promise从而判断是否成功
      try {
